...
จาก `bench_fps.py` บนเครื่องจริง: **คอขวดคือ `--delay` ไม่ใช่จำนวน packet**

| ปรับอะไร | ผล |
|---|---|
| ตัด packet 19 → 16 reports (`--lean`) | +12% |
| ลด `--delay` 8.5 → 0 | 5.3 → 55.7 FPS |

**แต่อย่าเพิ่งรีบลด delay** — รันซ้ำรอบสองแล้ว delay 1.0 ms ทำ **endpoint ค้างถาวร**
ส่งอะไรต่อไม่ได้จนกว่าจะถอดสาย USB เสียบใหม่ (รอบแรกลงถึง 0 ms ยังรอด = อาการเป็นแบบสุ่ม)
นี่คือสาเหตุจริงของอาการ "สตรีมหยุดกลางทาง"

หาค่าที่ใช้ได้จริงต้องทดสอบทีละค่ายาว ๆ ไม่ใช่กวาดเร็ว ๆ:

```bash
python bench_fps.py --soak 4 --seconds 180
```

และ *throughput ผ่านไม่ได้แปลว่าภาพไม่กระพริบ* — การกระพริบเกิดฝั่งเฟิร์มแวร์
ที่เอาเฟรมซึ่งยังมาไม่ครบไปแสดง โปรแกรมจับไม่ได้ ต้องดูด้วยตา:

```bash
python bench_fps.py --visual
```
