...
## นาฬิกาในตัวคีย์บอร์ด (ไม่ต้องสตรีมเลย)

คีย์บอร์ดมี **RTC ในตัว** และแสดงนาฬิกาเองได้ — ค้นพบจากการจับ USB
โปรแกรมทางการส่งคำสั่ง `04 28` ตั้งเวลาให้ตอนเปิดโปรแกรม

```bash
python -m amg65 set-time
```

**นี่คือวิธีที่ดีที่สุดถ้าอยากได้นาฬิกาเปิดทิ้งไว้** เพราะไม่มีทราฟฟิก USB เลย:

| | สตรีมนาฬิกาจาก PC | นาฬิกาในเฟิร์มแวร์ |
|---|---|---|
| endpoint ค้าง | เป็นไปได้ | **เป็นไปไม่ได้** |
| ต้องเปิดโปรแกรมค้าง | ต้อง | ไม่ต้อง |
| ปิดคอมแล้ว | ดับ | **ยังเดิน** |

แอป tray ซิงค์เวลาให้อัตโนมัติทุกครั้งที่เปิด สลับไปโหมดนาฬิกาของคีย์บอร์ดได้จากปุ่มบนตัวเครื่อง
