...
| `--speed 0x0C` | ไบต์ที่ยังไม่รู้ความหมายใน header (ทางการใช้ 0x0C) |
| `--preview` / `--no-upload` | ดูก่อน / ไม่ต้องส่งเข้าเครื่อง |

ทดสอบแล้วถึง **255 เฟรม / 241 KB / 59 ก้อน** ผ่านหมด — **ไม่มีเพดานฝั่งฮาร์ดแวร์**
255 คือค่าสูงสุดที่ส่งได้อยู่แล้วเพราะจำนวนเฟรมเก็บในไบต์เดียวของ header
เท่ากับเก็บแอนิเมชันยาว ~25 วินาทีไว้ในคีย์บอร์ดได้โดยไม่ต้องต่อคอม

⚠️ ถ้าอัปโหลดแล้วภาพมั่ว **ให้อัปโหลดซ้ำอีกครั้ง** — เจอครั้งเดียวตอนอัปโหลด
แอนิเมชันหลายเฟรมต่อจากภาพนิ่งเฟรมเดียว เดาว่าเฟิร์มแวร์ไม่ได้เคลียร์บัฟเฟอร์เก่า
