...
# จังหวะระหว่างเฟรมของ live stream
# โปรแกรมทางการเว้น 111 ms (9 FPS) — วัดจากการจับ USB — แต่นั่นเป็นค่าที่มันเลือก
# ตอนยังไม่มี ACK flow control เมื่อเรารอ ACK ทุก packet แล้ว (ได้ flow control จริง)
# ลดช่องว่างลงได้ ทดสอบไล่หาเพดาน + soak 5 นาที + ตายืนยันว่าไม่มี dot ผิดตำแหน่ง:
#     111 ms =  9.0 FPS  (ค่าเดิม/ทางการ)
#      80 ms = 12.4 FPS
#      60 ms = 16.5 FPS  ผ่าน soak 5 นาที / 4,953 เฟรม / ACK พลาด 0 / ภาพสะอาด
#      45 ms = 19.7 FPS  <- ผ่าน soak 5 นาที / 5,900 เฟรม / ACK พลาด 0 / ภาพสะอาด
# ยิงติดกันไม่เว้นเลย (~18.5 FPS) ยังค้างใน 13 วินาที เพราะเฟิร์มแวร์วาดจอไม่ทัน
# 45 ms คือเร็วสุดที่ soak ยาวแล้วยังนิ่ง (เร็วกว่าทางการ 2.2 เท่า) ตั้งเป็น default
MIN_FRAME_INTERVAL = 0.045
