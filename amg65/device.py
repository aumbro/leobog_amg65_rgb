...
# ไม่มีปัญหาไฟล์ล็อกค้างตอนโปรแกรมถูกฆ่า และยังใช้เป็นช่องส่งสัญญาณ "ปิด" ได้ในตัว
_LOCK_PORT = 47865
_lock_socket = None
...
def claim_exclusive() -> None:
    ...
    try:
        ...
        raise AlreadyRunning(
            ...
            "  ปิดตัวเดิมก่อน (ใช้ `python -m amg65 stop` หรือเมนู 'ออก' ของ tray)"
        )
    _lock_socket = sock  # ถือไว้ตลอดอายุโปรเซส


def listen_for_stop(callback) -> None:
    """รับสัญญาณ 'ปิด' จากคำสั่ง stop แล้วเรียก callback ให้ปิดตัวเองอย่างสะอาด

    ใช้ล็อก socket ที่ถืออยู่แล้วเป็นช่องรับ — ใครก็ตามที่ connect เข้ามาคือคำสั่งปิด
    callback ควรสั่งหยุดลูปวาดแล้วปิด HID handle ให้เรียบร้อย **ก่อน** โปรเซสตาย
    เพราะการตายตอนกำลังเขียน MI_02 อยู่คือสาเหตุที่ทำให้ endpoint ค้าง
    """
...
def request_stop(timeout: float = 6.0) -> str:
    """สั่งให้โปรแกรม amg65 ที่รันอยู่ปิดตัว คืนผลเป็นข้อความสถานะ

    ส่งสัญญาณผ่าน socket ก่อน (ให้ปิดสะอาด) ถ้าไม่ยอมปิดค่อย force kill เป็นทางสุดท้าย
    """
    import socket
    import time
    ...
    try:
        ...
        return "idle"  # ไม่มีใคร bind port อยู่ = ไม่มีโปรแกรมรัน

    # รอจน port ว่าง = โปรเซสเก่าปิดสะอาดและคืน port แล้ว
    deadline = time.perf_counter() + timeout
    while time.perf_counter() < deadline:
        check = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            check.bind(("127.0.0.1", _LOCK_PORT))
            check.close()
            return "stopped"
        except OSError:
            check.close()
    ...
    # ปิดสะอาดไม่สำเร็จ (accept thread อาจตายค้าง) — หา PID แล้ว force kill
    return "forced" if _force_kill_holder() else "stuck"


def _force_kill_holder() -> bool:
    """หาโปรเซสที่ถือ port ล็อกอยู่แล้วสั่งฆ่า (ทางสุดท้าย เมื่อปิดสะอาดไม่สำเร็จ)."""
    try:
        import psutil
    except ImportError:
        return False
    for conn in psutil.net_connections(kind="inet"):
        if (
            conn.laddr
            and conn.laddr.port == _LOCK_PORT
